...
ADD COLUMN payment_method text NOT NULL DEFAULT 'C' CHECK (payment_method IN ('C', 'V'));

-- Add comment for clarity
COMMENT ON COLUMN public.sales.payment_method IS 'Payment method: C for Cash, V for Virement (Bank Transfer)';

-- Update the notification generation function to only trigger for Flight Confirmed with registration
...
BEGIN
  -- Only generate notifications for Flight Confirmed with registration departing tomorrow
...
-- Update the generate_notifications function to only handle Flight Confirmed with registration
...
-- Add function to confirm bank transfer payment (for managers)
